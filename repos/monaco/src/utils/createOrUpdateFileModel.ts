...
export const createOrUpdateFileModel = (path: string, fileModel:TMFile) => {
  const model = window.monaco.editor.getModels().find(model => model.uri.path === path)
  model ? updateModel(model, fileModel.content) : createModel(path, fileModel.content)
}
