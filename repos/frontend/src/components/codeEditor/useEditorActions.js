 ...
 * @param {Object} editorRef - React ref object of the MonacoEditor
 *
 * @returns {function} - Callback to call when saving the active file
 */
const useRunAction = (activeFile, editorRef) => {
  return useCallback(
    async (event, testFile, testCommand, hasPending) => {
      ...
      const canRun =
        content !== testFile.content || hasPending
    ...
    [SCREENS.EDITOR, editorRef.current]
  )
}
