...
export const SelectDefinition = React.memo(props => {
  const { styles, step, selectAction } = props
  const { definitionTypes } = useSelector(CATEGORIES.DEFINITION_TYPES)
  const definitionsFromType =
    step.type && definitionTypes[step.altType || step.type]

  const selectStyles = useStyle('definitions.select', styles)

  return (
    <View className={`step-edit-select-main`} style={selectStyles?.main}>
      <Label className={`step-edit-select-label`} style={selectStyles?.label}>
        Definition
      </Label>
      <Select
      ...
      >
        {!step.definition && (
          <Option key={EMPTY_STEP} value={EMPTY_STEP} label={EMPTY_STEP} />
        )}
        {definitionsFromType &&
          definitionsFromType.map(parsed => {
            const { name, uuid } = parsed
            return <Option key={uuid} value={uuid} label={name} />
          })}
      </Select>
    </View>
  )
