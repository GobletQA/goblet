...
type TGConfig = Record<string, any>

// TODO: figure out how to fix this
const defConfig:TGConfig = require(path.join(
  path.join(GSHRoot, `../../`),
...
const loadConfigByType = (
  config:(...args:any[]) => TGConfig|TGConfig,
  ...args:any[]
  ) => {
 ...
 */
const addConfigFileTypes = (config:TGConfig) => {
 ...
 */
export const loadConfigFromFolder = (baseDir:string) => {
  return [
    '',
    './config',
    './configs',
    ...
    './test',
    './tests'
  ].reduce((found:any, loc) => found || getConfigAtPath(path.join(baseDir, loc) || found),
    false
  )
}
 ...
 */
export const getConfigAtPath = (pathToCheck:string) => {
 ...
 */
const loadConfigFromBase = (base:string) => {
  const {
    GOBLET_CONFIG_BASE,
    GOBLET_RUN_FROM_CI,
  } = process.env
