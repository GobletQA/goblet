        ...
        actions?.map(act => {
          ...
          return (
            <ActionComp
              ...
              children={children}
              key={`${title}-${id ?? name}`}
              className={`goblet-race-panel-header-icon ${className}`.trim()}
              {...iconProps}
            />
          )
        }).filter(Boolean)
...
export const PanelHeader = (props:TPanelHeader) => {
  ...
  return (
    <Container
      onClick={onCollapse}
      className={cls(`goblet-race-sidebar-panel-header`, closed ? `closed` : ``)}
    >
      <Arrow
        collapse={closed}
        className={`goblet-race-panel-toggle-icon`}
      />
