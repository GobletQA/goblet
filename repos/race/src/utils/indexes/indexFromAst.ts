...
export type TIndexFromAst = Omit<TPatchFeatureOpts, `indexes`> & {
  indexes: TRaceIndex
}
...
const updateChildIndex = (props:TIndexFromAst, nextIdx:number) => {
  ...
  child.index = nextIdx
  indexes.splice(nextIdx, 0, { ast: child, parent } as TRaceIndexItem)
 ...
 */
const reIndex = (indexes:TRaceIndex, refIndex:number, amount:number=1) => {
